    ...
    *,
    ...
    if not sumo_config.is_file():
        raise FileNotFoundError(
            ...
            "Apply/build the minimal SUMO scenario first."
        )
    ...
    try:
        for step_index in range(steps):
            ...
            simulation_time_ms = int(round(traci.simulation.getTime() * 1000))
            vehicle_ids = list(traci.vehicle.getIDList())
            observed.update(vehicle_ids)

            if SUBJECT_ID not in vehicle_ids:
                continue

            x_m, y_m = traci.vehicle.getPosition(SUBJECT_ID)
            state = VehicleState(
                vehicle_id=SUBJECT_ID,
                simulation_time_ms=simulation_time_ms,
                x_m=float(x_m),
                y_m=float(y_m),
                lane_position_m=float(traci.vehicle.getLanePosition(SUBJECT_ID)),
                speed_mps=float(traci.vehicle.getSpeed(SUBJECT_ID)),
                acceleration_mps2=float(traci.vehicle.getAcceleration(SUBJECT_ID)),
                lane_id=str(traci.vehicle.getLaneID(SUBJECT_ID)),
                road_id=str(traci.vehicle.getRoadID(SUBJECT_ID)),
                route_edges=tuple(str(edge) for edge in traci.vehicle.getRoute(SUBJECT_ID)),
                route_index=int(traci.vehicle.getRouteIndex(SUBJECT_ID)),
            )
            last_state = state
    ...
    if last_state is None:
        raise RuntimeError(
            f"{SUBJECT_ID} was not observed during {steps} TraCI steps. "
            f"Observed IDs: {sorted(observed)}"
        )
    return last_state, sorted(observed)
...
) -> tuple[GossipNetworkEmulator, list[DetectionRecord]]:
...
def run_protocol(
    *,
    ...
    background_metrics = run_background_load(
        count=max(0, args.vehicle_count - len(observed_vehicle_ids)),
        factory=factory,
    ...
    emulator, gossip_detections = asyncio.run(
        exchange_gossip(
            witnesses=witnesses,
...
def main() -> int:
    ...
    try:
        state, observed_vehicle_ids = collect_subject_state(
        ...
        base_result: dict[str, Any] = {
            ...
            "vehicle_count_mode": (
                "three physical SUMO vehicles plus logical protocol background load"
            ),
            "witness_count": args.witness_count,
            "slot_count": args.slots,
            "slot_ms": args.slot_ms,
        ...
        print(f"Logical load       : {args.vehicle_count}")
