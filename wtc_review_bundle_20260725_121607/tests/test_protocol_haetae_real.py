...
from wtc.scenarios_haetae import build_fixture, default_dll_path


DLL_PATH = default_dll_path()
MSYS2_RUNTIME = Path(r"C:\msys64\ucrt64\bin")

pytestmark = pytest.mark.skipif(
    os.name != "nt" or not DLL_PATH.is_file() or not MSYS2_RUNTIME.is_dir(),
    reason="real HAETAE Windows DLL/runtime is unavailable",
)


def test_real_haetae_detects_equivocation_and_serializes_evidence() -> None:
    factory, witness, private_key = build_fixture(DLL_PATH)

    c0 = factory.create(
        subject_id="Vehicle-A",
        session_id="REAL-HAETAE-TEST",
        epoch=1,
        sequence=0,
    ...
    assert isinstance(c1a.signature, str)
