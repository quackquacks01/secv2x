...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=500,
        help=(
            "Maximum TraCI steps used to wait for all requested physical "
            "vehicles to enter the network. The best (largest) active snapshot "
            "is retained instead of checking only the final fixed step."
        ),
    )
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.activation_wait_steps <= 0:
        raise ValueError("activation-wait-steps must be positive")
...
def resolve_route_files_from_sumocfg(sumo_config: Path) -> list[Path]:
    """Resolve route-files referenced by a SUMO configuration."""

    config_root = ET.parse(sumo_config).getroot()
    route_node = config_root.find("./input/route-files")
    if route_node is None:
        return []

    raw_value = route_node.attrib.get("value", "")
    paths: list[Path] = []
    for item in raw_value.split(","):
        ...
        if not path.is_absolute():
            path = (sumo_config.parent / path).resolve()
        paths.append(path)
    return paths


def count_explicit_route_vehicles(sumo_config: Path) -> tuple[int, list[Path]]:
    """
    Count explicit <vehicle> elements in every route file.

    The scalable generator uses explicit vehicles rather than <flow>, so this
    provides an early and precise stale-file check before SUMO is started.
    """

    route_files = resolve_route_files_from_sumocfg(sumo_config)
    total = 0
    for route_file in route_files:
        if not route_file.is_file():
            raise FileNotFoundError(
                f"SUMO route file referenced by config does not exist: {route_file}"
            )
        route_root = ET.parse(route_file).getroot()
        total += len(route_root.findall("vehicle"))
    return total, route_files
...
def collect_vehicle_snapshot(
    *,
    ...
    activation_wait_steps: int = 500,
) -> tuple[VehicleState, dict[str, VehicleState], list[str]]:
    """
    Collect a SUMO vehicle snapshot.

    For physical-scaling runs, vehicles with the same departure time may not
    all be inserted during the first small fixed window. Therefore this
    function retains the largest active snapshot and waits up to
    activation_wait_steps for the requested count.
    """
    ...
    traci.start(command)
    best_snapshot: dict[str, VehicleState] = {}
    ...
    total_steps = steps
    if minimum_active_vehicles is not None:
        total_steps = max(steps, activation_wait_steps)
    ...
    try:
        for step_index in range(total_steps):
            ...
            if len(snapshot) > len(best_snapshot):
                best_snapshot = snapshot

            try:
                last_diagnostics = {
                    "loaded": int(traci.simulation.getLoadedNumber()),
                ...
                last_diagnostics["step_index"] = step_index
                last_diagnostics["simulation_time_ms"] = simulation_time_ms
            ...
            if print_state and SUBJECT_ID in snapshot:
                ...
                print(
                    ...
                    f"max_active={len(best_snapshot)}"
                )

            if (
                minimum_active_vehicles is not None
                and len(best_snapshot) >= minimum_active_vehicles
                and SUBJECT_ID in best_snapshot
            ):
                break
    finally:
        traci.close()

    if SUBJECT_ID not in best_snapshot:
        raise RuntimeError(
            f"{SUBJECT_ID} was not active in any collected snapshot. "
    ...
    if (
        ...
        raise RuntimeError(
            "physical SUMO vehicle activation timeout: "
            f"requested={minimum_active_vehicles}, "
            f"max_active={len(best_snapshot)}, "
            f"observed_total={len(observed)}, "
            f"wait_steps={total_steps}, "
            f"last_loaded={last_diagnostics['loaded']}, "
            f"last_departed={last_diagnostics['departed']}, "
            f"remaining_expected={last_diagnostics['remaining_expected']}"
        )

    return (
        best_snapshot[SUBJECT_ID],
        best_snapshot,
        sorted(observed),
    )
...
def main() -> int:
    ...
    try:
        route_vehicle_count: int | None = None
        route_files: list[Path] = []
        if args.require_physical_vehicles:
            route_vehicle_count, route_files = count_explicit_route_vehicles(
                args.sumo_config
            )
            if route_vehicle_count < args.vehicle_count:
                raise RuntimeError(
                    "physical SUMO route file does not contain enough vehicles: "
                    f"requested={args.vehicle_count}, "
                    f"route_xml_count={route_vehicle_count}, "
                    f"route_files={[str(path) for path in route_files]}"
                )
        ...
        state, vehicle_states, observed_vehicle_ids = collect_vehicle_snapshot(
            ...
            minimum_active_vehicles=(
                args.vehicle_count
                if args.require_physical_vehicles
                else None
            ),
            activation_wait_steps=args.activation_wait_steps,
        )
        ...
        base_result: dict[str, Any] = {
            ...
            "route_xml_vehicle_count": route_vehicle_count,
