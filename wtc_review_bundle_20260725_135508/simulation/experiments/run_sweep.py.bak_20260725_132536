...
def validate_physical_scenario(
    *,
    ...
    """
    Validate that a cached scalable SUMO scenario really matches the requested
    physical vehicle and Witness counts.

    This prevents an old N10 scenario from being silently reused for N50/N100.
    """

    sumo_config = output_dir / "scalable.sumocfg"
    network_file = output_dir / "scalable.net.xml"
    route_file = output_dir / "scalable.rou.xml"
    manifest_file = output_dir / "scenario_manifest.json"

    required = [sumo_config, network_file, route_file, manifest_file]
    ...
    try:
        ...
        return False, f"invalid manifest: {type(exc).__name__}: {exc}"

    if int(manifest.get("vehicle_count", -1)) != vehicle_count:
        return (
            False,
            "manifest vehicle_count mismatch: "
            f"expected={vehicle_count}, actual={manifest.get('vehicle_count')}",
        )
    if int(manifest.get("witness_count", -1)) != witness_count:
        return (
            False,
            "manifest witness_count mismatch: "
            f"expected={witness_count}, actual={manifest.get('witness_count')}",
        )

    manifest_vehicles = manifest.get("vehicles")
    if not isinstance(manifest_vehicles, list):
        return False, "manifest vehicles is not a list"
    if len(manifest_vehicles) != vehicle_count:
        return (
            False,
            "manifest vehicle list mismatch: "
            f"expected={vehicle_count}, actual={len(manifest_vehicles)}",
        )

    try:
        route_root = ET.parse(route_file).getroot()
    except Exception as exc:
        return False, f"invalid route XML: {type(exc).__name__}: {exc}"
    ...
    if len(vehicle_elements) != vehicle_count:
        return (
            False,
            "route vehicle count mismatch: "
            f"expected={vehicle_count}, actual={len(vehicle_elements)}",
        )

    required_ids = {"Vehicle-A"} | {
        f"Witness-W{index}"
        for index in range(1, witness_count + 1)
    }
    missing_ids = sorted(required_ids - route_vehicle_ids)
    if missing_ids:
        return False, f"route missing required IDs: {missing_ids}"

    return True, "scenario files match requested counts"
...
def ensure_physical_scenario(
    *,
    ...
    valid, validation_reason = validate_physical_scenario(
    ...
    if regenerate or not valid:
        ...
        print(
            "[SCENARIO REBUILD] "
            f"N={vehicle_count}, W={witness_count}: {reason}"
        )
        ...
        if not valid:
            raise RuntimeError(
                "generated physical SUMO scenario failed validation: "
                f"N={vehicle_count}, W={witness_count}, "
