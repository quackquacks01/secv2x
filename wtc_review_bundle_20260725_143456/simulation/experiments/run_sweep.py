...
def validate_physical_scenario(
    *,
    ...
    """Validate a cached physical SUMO scenario before it is reused."""
    ...
    route_vehicle_ids = {
        ...
        for element in route_vehicles
    }
    if len(route_vehicles) != vehicle_count:
        return (
            False,
            "route vehicle count mismatch: "
            f"expected={vehicle_count}, actual={len(route_vehicles)}",
        )

    required_vehicle_ids = {"Vehicle-A"} | {
        f"Witness-W{index}"
        for index in range(1, witness_count + 1)
    }
    missing_ids = sorted(required_vehicle_ids - route_vehicle_ids)
    if missing_ids:
