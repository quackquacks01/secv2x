...
from .crypto_aimer import AIMer128fSigner
...
def default_dll_path() -> Path:
    ...
    return project_root / "native" / "aimer" / "libaimer-128f.dll"
...
def build_fixture(
    ...
    signer = Base64SignerAdapter(AIMer128fSigner(path))
...
def main() -> None:
    factory, witness, private_key = build_fixture()

    c0 = factory.create(
        subject_id="Vehicle-A",
        session_id="AIMER-MERGE-001",
        epoch=1,
        sequence=0,
    ...
    print_result("1. AIMer initial commitment", r1)
    ...
    print_result("2. AIMer valid re-commitment", r2)
    ...
    print_result("3. AIMer equivocation", r3)

    tampered = replace(c1a, merkle_root="ff" * 32)
    r4 = witness.process(tampered)
    print_result("4. AIMer tampered commitment", r4)
